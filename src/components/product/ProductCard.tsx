...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden border border-border-soft bg-white shadow-[0_18px_40px_rgba(18,18,18,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_50px_rgba(18,18,18,0.1)]">
      ...
      >
        {tag ? (
          <span className="absolute left-4 top-4 z-10 bg-primary px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] text-white">
            {tag}
          </span>
        ) : null}
        <img
      ...
      <div className="flex flex-1 flex-col px-6 pb-6 pt-5 text-center">
        ...
        <Link to={`/san-pham/${product.slug}`} className="mt-3">
          <h3
            className="text-2xl leading-tight text-dark transition group-hover:text-primary"
